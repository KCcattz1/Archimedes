...
const templates = {
    ...
    nextTier: (roleName, faction, direction, title, requiredRep) => `Noble **${roleName}**, the next tier and title for **${faction}** in the **${direction}wards** direction is **${title}**. You require **${requiredRep}** more reputation points to transcend this tier.`,
...
client.on(Events.InteractionCreate, async interaction => {
    ...
    if (interaction.isAutocomplete()) {
        ...
        try {
            if (focusedOption.name === 'role') {
                ...
                const allRepValues = await getAllRepValues(process.env.SPREADSHEET_ID, roleName);
    ...
    if (interaction.isCommand()) {
        if (commandName === 'archimedes' && options.getSubcommand() === 'rep') {
            ...
            const repValue = await getRepValue(process.env.SPREADSHEET_ID, roleName, faction);

            if (repValue !== null) {
                const factionData = await getFactionData(process.env.SPREADSHEET_ID, faction);
        ...
        if (commandName === 'archimedes' && options.getSubcommand() === 'durendalrep') {
            ...
            const filteredFactions = await getFilteredFactions(process.env.SPREADSHEET_ID);
        ...
        if (commandName === 'archimedes' && options.getSubcommand() === 'repupdate') {
            ...
            try {
                const response = await updateData(process.env.SPREADSHEET_ID, roleName, faction, deltaValue);
                await interaction.reply({
    ...
    if (interaction.isButton()) {
        ...
        const combinedRepValue = await getCombinedRepValue(process.env.SPREADSHEET_ID, faction);
        if (combinedRepValue !== null) {
            const diplomacyData = await getFactionData(process.env.SPREADSHEET_ID, 'DIPLOMACY');
